...
root_dir = ['en_wiki_gpt2_nucleus_sampling_on_wikitext']
...
for name, data in zip(root_dir, dataset):
    ...
    bad_index = []
    for idx, (i, l) in enumerate(zip(data, index)):
        if i == 2:
            tie_num += 1
            continue
        if i == l:
            win_num += 1
            bad_index.append(idx + 1)
        else:
            loss_num += 1
    ...
    bad_index = set(bad_index)
    print(bad_index, len(bad_index))
